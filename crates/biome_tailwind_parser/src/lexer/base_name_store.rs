...
impl BaseNameStore {
    ...
    /// Convenience one-shot API that returns the final base end, including dashed basename + boundary rules.
    #[inline]
    pub(crate) fn match_base_end(&self, text: &[u8]) -> usize {
        self.matcher(text).base_end()
    }
}
...
impl<'s, 't> BaseNameMatcher<'s, 't> {
    pub(crate) fn base_end(&self) -> usize {
        ...
        if let Some(end) = best_end {
            if self.text.get(end).map_or(true, |b| is_boundary_byte(*b)) {
                return end;
            }
        }
...
mod tests {
    ...
    fn matches_longest_prefix() {
        ...
        assert_eq!(store.match_base_end(b"border-t-red-300"), "border-t".len());
        assert_eq!(
            store.match_base_end(b"mask-radial-at-top"),
            "mask-radial-at".len()
        );
        // Does not match undashed names that aren't present
        assert_eq!(store.match_base_end(b"bg-red-500"), "bg".len());
    }

    #[test]
    fn respects_delimiters() {
        let store = BaseNameStore::new(&["nth-last", "rounded-bl"]);
        assert_eq!(store.match_base_end(b"nth-last:odd"), "nth-last".len());
        assert_eq!(store.match_base_end(b"rounded-bl-lg"), "rounded-bl".len());
    }
    ...
    fn stops_on_non_edge() {
        ...
        assert_eq!(store.match_base_end(b"border-tfoo"), "border".len());
